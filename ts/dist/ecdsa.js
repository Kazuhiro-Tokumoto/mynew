export class PointPairSchnorrP256 {
    ...
    addJJ(P1, Q) {
        ...
        if (Z2 === 0n)
            return P1;
        if (Z2 === 1n)
            return this.addMJ(P1, X2, Y2);
        if (Z1 === 1n)
            return this.addMJ(Q, X1, Y1);
        const p = this.P;
        const Z1Z1 = (Z1 * Z1) % p;
        const Z2Z2 = (Z2 * Z2) % p;
        const U1 = (X1 * Z2Z2) % p;
    ...
    scalarMultGJac(k) {
        ...
        let R = [...this.G_precomp_window[0][win0]];
    ...
    scalarMult(Pt, k) {
        ...
        let R = [0n, 1n, 0n];
        let addend = [Pt[0], Pt[1], 1n];
        while (k > 0n) {
            if (k & 1n)
                R = this.addJJ(R, addend);
    ...
    inv(a, m) {
        ...
        if (r !== 1n)
            throw new Error("inv: not invertible");
        if (t < 0n)
            t += m;
        return t;
    }
    ...
    generateK(message, privateKey) {
        ...
        let V = new Uint8Array(qLen).fill(0x01), K = new Uint8Array(qLen).fill(0x00);
...
async function test() {
    ...
    const ecSig = await crypto.subtle.sign({ name: "ECDSA", hash: "SHA-256" }, ecKeyPair.privateKey, message);
    const ecSignSamples = [];
    for (let i = 0; i < ITERATIONS; i++) {
        const t0 = performance.now();
        await crypto.subtle.sign({ name: "ECDSA", hash: "SHA-256" }, ecKeyPair.privateKey, message);
        ecSignSamples.push(performance.now() - t0);
    }
    const ecVerifySamples = [];
    for (let i = 0; i < ITERATIONS; i++) {
        const t0 = performance.now();
        await crypto.subtle.verify({ name: "ECDSA", hash: "SHA-256" }, ecKeyPair.publicKey, ecSig, message);
    ...
    console.log("=".repeat(50));
    console.log(`自作 vs WebCrypto ECDSA  署名: ${(ss.mean / es.mean).toFixed(1)}倍   検証: ${(sv.mean / ev.mean).toFixed(1)}倍`);
}
// ================================================================
//  正当性チェック
// ================================================================
...
const fakeResult = dsa.verify(encoder.encode("Fake message!"), publicKey, signature);
console.time("verify");
